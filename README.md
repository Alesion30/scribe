...
削除の対象は scribe が `~/.scribe/recordings/` に自動生成した WAV だけ。録音に失敗したとき、無音だったとき、文字起こしが失敗したとき、途中で止めたときは削除せず、復旧や再実行に使える状態で残す。削除できなかった場合は、文字起こしが済んでいることと残っている WAV のパスをエラーとして表示する。
